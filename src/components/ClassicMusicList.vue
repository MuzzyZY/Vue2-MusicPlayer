    ...
    <keep-alive>
      ...
      <div class="musicList">
        <van-swipe :loop='false' width="100" :show-indicators='false' class="list-swipe" height="4.93rem">
          <van-swipe-item v-for="item in classicSongList" :key="item.id">
...
<script>
import { getClassicMusicList } from '@/request/item.js'
export default {
  data() {
    return {
      classicSongList: []
  ...
  created() {
    getClassicMusicList().then(res => {
      this.classicSongList = res.data.playlists.slice(1, 21)
    })
  },
  methods: {
    changeCount(num) {
